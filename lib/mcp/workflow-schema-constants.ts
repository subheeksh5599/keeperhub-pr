...
export const SYSTEM_ACTIONS = {
  Condition: {
    ...
    requiredFields: {
      condition:
        'string - JavaScript expression using {{@nodeId:Label.field}} syntax. Supported operators: == (soft equals), === (equals), != (soft not equals), !== (not equals), >, >=, <, <=, contains, startsWith, endsWith, matchesRegex, isEmpty, isNotEmpty, exists (not null and not undefined), doesNotExist (null or undefined), isNull (=== null), isNotNull (!== null), isUndefined (=== undefined), isNotUndefined (!== undefined). Use == for cross-type comparisons (e.g., string "0" vs number 0). matchesRegex takes its pattern as a quoted string literal, not a reference, and refuses a pattern that applies a quantifier to a group containing a quantifier or an alternation, or two quantifiers in a row over the same characters (a+a+). Example: "{{@check-balance:Check Balance.balance}} == 0"',
    },
    optionalFields: {
      conditionConfig:
        'object - Visual condition builder config. Structure: { group: { id: "nanoid", logic: "AND" | "OR", rules: [{ id: "nanoid", leftOperand: "{{@nodeId:Label.field}}", operator: "===" | "==" | "!==" | "!=" | ">" | ">=" | "<" | "<=" | "contains" | "startsWith" | "endsWith" | "isEmpty" | "isNotEmpty" | "exists" | "doesNotExist" | "isNull" | "isNotNull" | "isUndefined" | "isNotUndefined" | "matchesRegex", rightOperand: "value" }] } }. Every group and rule MUST have a unique id. Operator must be the exact symbol (e.g. "===" not "equals", "<" not "less_than"). When provided, the condition expression is auto-generated from this visual config. A matchesRegex rightOperand must be a quoted string literal, and a pattern that applies a quantifier to a group containing a quantifier or an alternation, or two quantifiers in a row over the same characters, is refused.',
    },
    outputFields: {
